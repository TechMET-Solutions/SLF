...
} from "../API/Master/User_Management/Roles";
...
const UserRolePermission = () => {
  ...
  const fetchRoles = async (page = 1) => {
    ...
    try {
      ...
      if (result?.roles) {
        ...
        setShowPagination(result.total > itemsPerPage);
      } else {
        setRoles([]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Prepare payload (important for backend consistency)
      const payload = {
        ...formData,
        valuation_amount: Number(formData.valuation_amount) || 0,
      };

      if (isEditMode) {
        await updateRolesApi(payload);
      } else {
        await addRolesApi(payload);
      }
      ...
      // ✅ Reset form (YOU MISSED valuation_amount BEFORE)
      setFormData({
        id: null,
        role_name: "",
        system_name: "",
        valuation_amount: "", // ✅ MUST RESET
        is_system_role: false,
        is_active: true,
      });
  ...
  const handleEdit = (role) => {
    setFormData({
      ...
      valuation_amount: role.valuation_amount || "", // ✅ ADD THIS
          ...
          <table className="w-full border-collapse">
            <thead className="bg-[#0A2478] text-white text-sm">
              <tr>
                ...
                <th className="px-4 py-2 text-left border-r border-gray-300 text-[13px] w-[120px]">
                  Valuation Amount
                </th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="px-4 py-2">
                      {(userData?.isAdmin ||
                        ...
                        >
                          <div
                            className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform  cursor-pointer duration-300 ease-in-out ${
                              row.is_active === 1
                                ? "translate-x-6"
                                : "translate-x-0"
                            }`}
