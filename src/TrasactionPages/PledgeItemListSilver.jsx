...
const PledgeItemListSilver = ({ rows, setRows, selectedScheme }) => {
  ...
  useEffect(() => {
    const fetchGoldRate = async () => {
      try {
        ...
        setGoldRate(response.data.data);
        setLoading(false);
      } catch (err) {
      ...
      try {
        ...
        if (response.data?.data) {
          ...
          const goldItemNames = allItems
            .filter((item) => item.code.toLowerCase().includes("silver"))
            .map((item) => item.name);
  ...
  //   // Validation
  //   if (field === "netWeight" && updatedValue > currentRow.gross) {
  ...
  //     currentRow.valuation = Math.round(quantity * weight * actualRate);
  ...
  const handleChange = (index, field, value) => {
    ...
    const updatedRows = rows.map((row, i) => {
      debugger;
      if (i !== index) return row; // ✅ keep other rows unchanged

      const currentRow = { ...row }; // ✅ clone only this row
      ...
      // ✅ Handle Assigned Purity (IMPORTANT FIX)
        ...
        currentRow.assignedPurityPercent =
          parseFloat(value?.purity_percent) || 0;
      } else {
        currentRow[field] = updatedValue;
      }
      ...
      if (
        ...
        const purityPercent = value.purity_percent
      ...
      return currentRow; // ✅ return updated row only
    });
                ...
                <td className="px-4 py-2">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      handleChange(index, "purity", selected); // ✅ pass full object
                    }}
