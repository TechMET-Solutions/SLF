...
const EditGoldLoanApplication = () => {
  ...
  const validateLoanForm = () => {
    if (!selectedCustomer?.id) return "Borrower is required";
    if (!selectedCoBorrower?.id) return "Co-Borrower is required";
  ...
  const fetchLoanData = async () => {
    ...
    try {
      ...
      setSearchTermForCoBorrower(
        `${data.coborrower_printName} (Id - ${data.CoBorrowerId})`,
      );
          ...
          <div className="flex gap-2 mt-2 p-1">
            ...
            <div className="flex flex-col">
              ...
              <select
                ...
                className="border border-gray-300 rounded-[8px] px-2 text-xs h-[30px] mt-1 bg-white w-[180px]"
              >
                ...
                {activeEmployees?.length > 0 ? (
                  ...
                  <option disabled>No valuers available</option>
                )}
              </select>
            </div>
